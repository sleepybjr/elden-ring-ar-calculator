...
function filterBySearchedWeapons(searchedWeapons) {
    return function (row) {
        return searchedWeapons.includes(row.weaponname);
    };
};

function filterByWeaponTypes(weaponTypes) {
    return function (row) {
        return weaponTypes.includes(row.weaponType);
    };
};

function filterByAffinityTypes(affinityTypes) {
    return function (row) {
...
function prepareData(searchedWeapons, weaponTypeFilter, affinityTypeFilter, somberFilter, smithingFilter, hideNoReqWeapons, levels, twoHanded) {
    const data = Table_Data;
    const searchedWeaponsFilter = searchedWeapons.map(row => row.label);
    const filteredDataSearched = data.filter(filterBySearchedWeapons(searchedWeaponsFilter));
    const filteredDataWeapon = data.filter(filterByWeaponTypes(weaponTypeFilter));
    const filteredDataSomber = somberFilter === true ? filteredDataWeapon : filteredDataWeapon.filter((weapon) => weapon.maxUpgrade !== 10 && weapon.maxUpgrade !== 0);
    const filteredDataSmithing = smithingFilter === true ? filteredDataSomber : filteredDataSomber.filter((weapon) => weapon.maxUpgrade !== 25);
    const filteredDataReqWeapons = hideNoReqWeapons === true ? filteredDataSmithing : filteredDataSmithing.filter((weapon) => highlightReqRow(weapon, levels, twoHanded) === false);

    const filteredDataAffinity = filteredDataSearched.concat(filteredDataReqWeapons).filter(filterByAffinityTypes(affinityTypeFilter));
...
function sortAlgorithm(data) {
    ...
    if (sort.direction !== null) {
        finalData = finalData.sort((a, b) => {
            if (new Set(['fullweaponname', 'weaponType', 'affinity', 'type1', 'type2']).has(sort.column)) {
                let sortedColumn = sort.column;

                if ('fullweaponname' === sort.column) {
                    sortedColumn = 'weaponname';
                }

                const nameA = a[sortedColumn] ? a[sortedColumn].toUpperCase() : 'Ω';
                const nameB = b[sortedColumn] ? b[sortedColumn].toUpperCase() : 'Ω';
                if (nameA < nameB) {
                    return -1;
                }
                if (nameA > nameB) {
                ...
                if (new Set(['type1', 'type2']).has(sort.column)) {
                    const second_sort = sort.column === 'type1' ? 'final_passive1' : 'final_passive2';
                    return b[second_sort] - a[second_sort];
                }

                return 0;
            } else if (new Set(['str_scaling_letter', 'dex_scaling_letter', 'int_scaling_letter', 'fai_scaling_letter', 'arc_scaling_letter']).has(sort.column)) {
                const A = a[sort.column];
                const B = b[sort.column];
...
export default function WeaponTable(props) {
    ...
    function onSort(sortedColumn) {
        ...
        sort = { column: sortedColumn, direction: newDirection }

        setSortedData(sortAlgorithm(unsortedData));
    };
    ...
    useEffect(() => { 
        let data = prepareData(props.searchedWeapons, props.weaponTypeFilter, props.affinityTypeFilter, props.somberFilter, props.smithingFilter, props.hideNoReqWeapons, props.levels, props.twoHanded);

        //calc data
        data.forEach((val) => {
            val.final_physical = Math.trunc(getPhyData(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_magic = Math.trunc(getMagData(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_fire = Math.trunc(getFireData(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_lightning = Math.trunc(getLighData(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_holy = Math.trunc(getHolyData(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_total_ar = Math.trunc(totalAR(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));
            val.final_sorcery_scaling = Math.trunc(getSorceryScaling(val, val.maxUpgrade, props.weaponLevels, props.levels, props.twoHanded));

            val.str_scaling_letter = getScalingLetter(val, val.maxUpgrade, props.weaponLevels, "str");
            val.dex_scaling_letter = getScalingLetter(val, val.maxUpgrade, props.weaponLevels, "dex");
            val.int_scaling_letter = getScalingLetter(val, val.maxUpgrade, props.weaponLevels, "int");
            val.fai_scaling_letter = getScalingLetter(val, val.maxUpgrade, props.weaponLevels, "fai");
            val.arc_scaling_letter = getScalingLetter(val, val.maxUpgrade, props.weaponLevels, "arc");

            val.final_passive1 = Math.trunc(getPassiveData(val, val.maxUpgrade, props.weaponLevels, props.levels));
            val.final_passive2 = Math.trunc(getPassiveData2(val, val.maxUpgrade, props.weaponLevels, props.levels));
        });
